...
class NationalTeamTacticDao extends DatabaseAccessor<AppDatabase> with _$NationalTeamTacticDaoMixin {
  ...
  Future<Map<String, Object?>?> getTacticByTeamIdWithTeam(int teamId) async {
    final query = select(nationalTeamTactics).join([
      innerJoin(db.nationalTeams, db.nationalTeams.id.equalsExp(nationalTeamTactics.teamId)),
    ])
  ...
  Future<int> updateTacticRecord(int teamId, NationalTeamTacticsCompanion tactic) {
    return (update(nationalTeamTactics)
  ...
  Future<int> deleteTactic(int teamId) {
    return (delete(nationalTeamTactics)
          ..where((t) => t.teamId.equals(teamId)))
        .go();
  }
  ...
  Future<int> deleteTacticById(int tacticId) {
    return (delete(nationalTeamTactics)..where((t) => t.id.equals(tacticId))).go();
  }
  ...
  Future<Map<int, NationalTeamTacticRecord>> getTacticsByTeamIds(List<int> teamIds) async {
    final tactics = await (select(nationalTeamTactics)
  ...
  Future<List<NationalTeamTacticRecord>> getTacticsByPresetName(String presetName) {
    return (select(nationalTeamTactics)
          ..where((t) => t.presetName.equals(presetName)))
        .get();
  }
  ...
  Future<Map<int, Map<String, Object?>>> getTacticsByTeamIdsWithTeam(List<int> teamIds) async {
    final query = select(nationalTeamTactics).join([
      innerJoin(db.nationalTeams, db.nationalTeams.id.equalsExp(nationalTeamTactics.teamId)),
    ])
