...
class PlayerMapper {
  ...
  static void validate(Player player) {
    ...
    if (player.heightCm <= 0 || player.heightCm > 250) {
      throw ArgumentError('Player height must be between 1cm and 250cm, got ${player.heightCm}');
    }
    if (player.weightKg <= 0 || player.weightKg > 200) {
      throw ArgumentError('Player weight must be between 1kg and 200kg, got ${player.weightKg}');
    }
  }
